...
let getComputedStyleX:(el:HTMLElement, name?:string)=>string;
...
function css(el:HTMLElement, name:string | {
  left?:number;
  top?:number;
  right?:number;
  bottom?:number;
  ...
  if (typeof name === 'object') {
    for (const i in name) {
      if (name.hasOwnProperty(i)) {
        css(el, i, name[i as keyof typeof name]);
      }
    }
    return undefined;
  }
...
function getDocument(node:Document | Node | Window) {
  if (isWindow(node)) {
...
function _getComputedStyle(elem:Element, name?: string | null, cs?:any) { // !!
...
function _getComputedStyleIE(elem:HTMLElement, name: string = "") {
  // currentStyle maybe null
  // http://msdn.microsoft.com/en-us/library/ms535231.aspx
  let ret:string = (elem as any)[CURRENT_STYLE] && (elem as any)[CURRENT_STYLE][name as string];
  ...
  if (_RE_NUM_NO_PX.test(ret) && !RE_POS.test(name)) {
    ...
    const rsLeft = (elem as any)[RUNTIME_STYLE][LEFT];

    // prevent flashing of content
    (elem as any)[RUNTIME_STYLE][LEFT] = (elem as any)[CURRENT_STYLE][LEFT];
    ...
    (elem as any)[RUNTIME_STYLE][LEFT] = rsLeft;
  }
  return ret === '' ? 'auto' : ret;
}
...
function getOffsetDirection(dir:"top" | "left", option:{
  useCssRight?:boolean;
...
function oppositeOffsetDirection(dir:"top" | "left" | 'right' | 'bottom'){
  if (dir === 'left') {
    return 'right';
  } else if (dir === 'right') {
    return 'left';
...
function each<T>(arr:T[], fn:(val:T)=>unknown) {
...
function swap(elem:HTMLElement, options, callback) {
  const old = {};
